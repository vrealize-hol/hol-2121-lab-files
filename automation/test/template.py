...
def get_pricing_card_details(id, cpu, mem, storage):
    fcnlog = ''
...
def q_1_1_1_1():
    ...
    log += f'Getting cloud zones and looking for a match with regex: {cz_match_name}\n'
...
def q_1_1_1_2():
    proj_match_name = '.*mercury'  # name of the project that user should have created
    ...
    log = f'Function started\n'
    log += f'Getting projects and looking for a match with regex: {proj_match_name}\n'
...
def q_1_1_1_3():
    pc_match_name = 'mercury'  # name of the project that user should have created
    pc_is_default = [{'email': 'Project Mercury Users@corp.local@corp.local'}]
    ...
    log = f'Function started\n'
    log += f'Getting pricing cards and looking for a match with regex: {pc_match_name}\n'
    pc_list, pc_ids, pc_count = get_pricing_cards(pc_match_name)
    if pc_count == 1: # found one match - proceed
        log += f'Found one match match in {pc_list}\n'
        pc_config, logs = get_pricing_card_details(pc_ids[0], pc_cpu, pc_mem, pc_storage)
        log += logs
